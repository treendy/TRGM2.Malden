	...
	if (!_bThisMissionCivsOnly) then {
		...
		if (_minimission) then {
			...
			if (_bSmallerAllOverPatrols) then {
				...
				if ((_bIsMainObjective && selectRandom [true,true,false]) || _moreEnemies) then {
				...
				if (_moreEnemies) then {
					...
					_trgCustomAIScript setTriggerArea [1250, 1250, 0, false];
					_trgCustomAIScript setTriggerActivation [TREND_FriendlySideString, format["%1 D", TREND_EnemySideString], true];
					_trgCustomAIScript setTriggerStatements ["this && TREND_SpottedActiveFinished", format["nul = [EAST, TREND_ReinforceStartPos1, %1, 3, true, true, true, true, false] spawn TREND_fnc_reinforcements; nul = [EAST, TREND_ReinforceStartPos2, %1, 3, true, true, true, false, false] spawn TREND_fnc_reinforcements;",str(_sidePos)], ""];
				};
			};
		};

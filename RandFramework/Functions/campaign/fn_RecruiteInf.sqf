...
if (_currentSpentPoints < (TREND_MaxBadPoints - TREND_BadPoints + 1)) then {
	...
	_SpawnedUnit addEventHandler ["killed",
		{
			//hint format["TEST: %1", TREND_CampaignRecruitUnitRifleman];
	...
	hint format[localize "STR_TRGM2_RecruiteInf_UnitAdd", name _SpawnedUnit,_currentSpentPoints+0.5,TREND_MaxBadPoints - TREND_BadPoints + 1];

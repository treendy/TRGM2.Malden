...
if (_durationOrCondition isEqualType {}) then {
	[_content, 86400, 10, _notificationIndex, _durationOrCondition] spawn TREND_fnc_handleNotification;
} else {
	[_content, _durationOrCondition, 10, _notificationIndex] spawn TREND_fnc_handleNotification;
};

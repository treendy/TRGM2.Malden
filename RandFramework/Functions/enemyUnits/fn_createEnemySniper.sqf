...
for "_i" from 1 to 20 do {
	if (!_foundPlace) then {
		...
		if (_cansee > 0) then {
			...
			TREND_SniperCount = TREND_SniperCount + 1;
		}
		else {
			...
			TREND_SniperAttemptCount = TREND_SniperAttemptCount + 1;
			if (_i < 11) then {
...
if (_foundPlace) then {
	while {alive(_spawnedUnit)} do {
		...
		TREND_SniperRevialTotal = count _enemies;
...
/*
TREND_SniperRevialTotal
TREND_SniperAttemptCount
TREND_SniperCount

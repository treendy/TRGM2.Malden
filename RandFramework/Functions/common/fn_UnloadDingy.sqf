...
if (str(_flatPos) == "[0,0,0]") then {
	...
	if (str(_flatPos) == "[0,0,0]") then {
		...
		[_dingy, [localize "STR_TRGM2_UnloadDingy_push",{[_this select 0, _this select 1, _this select 2, _this select 3] spawn TREND_fnc_PushObject;}]] remoteExec ["addAction", 0];

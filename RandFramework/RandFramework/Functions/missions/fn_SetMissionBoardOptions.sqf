...
switch (_option) do {
    ...
    case "NEW_MISSION": {
		{
			...
			if (_dCurrentRep == 5) then {
				...
				[_x, [localize "STR_TRGM2_SetMissionBoardOptions_RecruitSpotterS",{_this spawn TREND_fnc_RecruiteInf;},[TREND_CampaignRecruitUnitSpotter]]] remoteExec ["addAction", 0];
			};
	...
	case "CAMPAIGN_END": {
		...
		[endMissionBoard, [localize "STR_TRGM2_SetMissionBoardOptions_ShowRep",{[] spawn TREND_fnc_ShowRepReport;}]] remoteExec ["addAction", 0];
		[endMissionBoard2, [localize "STR_TRGM2_SetMissionBoardOptions_ShowRep",{[] spawn TREND_fnc_ShowRepReport;}]] remoteExec ["addAction", 0];

...
if (_useAceInteractionForTransport && [] call TREND_fnc_isAceLoaded) then {
	...
	_generateChildActions = {
		...
		_actionAdapterPickupAce = {
			...
			[_selectedVehicle,true] spawn TREND_fnc_selectLZ;
		};
		...
		private _actions = [];
		{
			...
			_actionName = [_x] call TREND_fnc_getTransportName;
	...
	_actionAdapterPickup = {
		...
		[_targetVehicle,true] spawn TREND_fnc_selectLZ;
	};
	...
	_playerActions = [];
	{
		...
		_name = [_x] call TREND_fnc_getTransportName;
	...
	{
		//[_x] remoteExec ["TREND_fnc_addPlayerActionPersistent",[0, -2] select isMultiplayer,true];
		[_x] remoteExec ["TREND_fnc_addPlayerActionPersistent",0,true];

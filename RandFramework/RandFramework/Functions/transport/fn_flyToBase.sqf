...
if ([_vehicle] call TREND_fnc_helicopterIsFlying) then {
	...
	_text = format [localize "STR_TRGM2_transport_radio_message_RTB",  [_vehicle] call TREND_fnc_getTransportName];
	[driver _vehicle,_text] call TREND_fnc_commsSide;
} else {
	_locationText = [position _vehicle,true] call TREND_fnc_getLocationName;
	_text = format [localize "STR_TRGM2_transport_fnflyToLz_EnterAirspace",  [_vehicle] call TREND_fnc_getTransportName, _locationText];
	[driver _vehicle,_text] call TREND_fnc_commsSide;
};
...
// Landing Comms
if ([_vehicle] call TREND_fnc_helicopterIsFlying) then  {
	_transportName = [_vehicle] call TREND_fnc_getTransportName;
...
[_vehicle,localize "STR_TRGM2_transport_fnflyToBase_BackHome"] call TREND_fnc_commsPilotToVehicle;

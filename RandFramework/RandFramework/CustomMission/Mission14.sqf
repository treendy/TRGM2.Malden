	...
	if ((_objInformant distance [_centralAO_x,_centralAO_y]) > 500 && selectRandom [true,false,false,false]) then {
		if (selectRandom [true,false]) then {
			...
			if (_flatPosSentry select 0 > 0) then {
				...
				[_checkPointGuidePos,_thisPosAreaOfCheckpoint,_thisAreaRange,_thisRoadOnly,_thisSide,_thisUnitTypes,_thisAllowBarakade,_thisIsDirectionAwayFromAO,true,UnarmedScoutVehicles,100] spawn TREND_fnc_setCheckpoint;
			};
		}
		else {

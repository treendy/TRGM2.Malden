...
MISSION_fnc_CustomMission = { //This function is the main script for your mission, some if the parameters passed in must not be changed!!!
	...
	*/
	...
	//###################################### Hack Data ######################################
	["Mission Setup: 8-0-9", true] call TRGM_GLOBAL_fnc_log;
	...
	if (random 1 < .50) then {
		...
		_objIED1 = selectRandom TRGM_VAR_IEDClassNames createVehicle [0,0,500];
		_objIED1 setVariable [_sIED1Name, _objIED1, true];
		missionNamespace setVariable [_sIED1Name, _objIED1];
		_fallbackPos = [_laptopPos, 5, 25, 10, 0, 0.3, 0,[],[_laptopPos vectorAdd [random 10, random 10, 0],_laptopPos vectorAdd [random 10, random 10, 0]], _objIED1] call TRGM_GLOBAL_fnc_findSafePos;
	...
	[_objLaptop1, [localize "STR_TRGM2_startInfMission_MissionTitle1", {_this spawn TRGM_GUI_fnc_downloadData;}, [localize "STR_TRGM2_downloadData_title", true, "TRGM_SERVER_fnc_hackIntel", []], 0, true, true, "", "_this isEqualTo player"]] remoteExec ["addAction", 0, true];

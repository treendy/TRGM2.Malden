...
// Objective Zones
{
	_markers pushBack ([_x, _radius] call TREND_fnc_selectLzCreateBolckedAreaMarker);
...
if (_baseReturnAllowed) then {
	_markers pushBack ([_baseLZPos ,_baseRadius , "ColorGreen" ] call TREND_fnc_selectLzCreateBolckedAreaMarker);
} else {
	_markers pushBack ([_baseLZPos vectorAdd [0.1,0.1,0],_baseRadius , "ColorWhite" ] call TREND_fnc_selectLzCreateBolckedAreaMarker);
};
...
if (!([_vehicle] call TREND_fnc_helicopterIsFlying)) then {
	...
	if (_baseLZPos distance2D _vehiclePositon > 100) then { // prevent double marker at base (colition with base restriction marker)
		_markers pushBack ([_vehiclePositon,_minimumDistance , "ColorWhite" ] call TREND_fnc_selectLzCreateBolckedAreaMarker);
	};
};



[_vehicle, _redZonePositions, _vehiclePositon,_baseLZPos, _radius,_minimumDistance, _markers,_baseRadius,_baseReturnAllowed,_isPickup] onMapSingleClick TREND_fnc_selectLZOnMapClick;

...
//			_airEscort = true;
//		};
//} forEach TREND_ClearedPositions;

_mainAOPos = TREND_ObjectivePossitions select 0;
if (! isNil "_mainAOPos") then {
	if (_mainAOPos in TREND_ClearedPositions  && (_mainAOPos distance2D _destinationPosition) < _radius) then {
		_airEscort = true;
	};
};
...
_mrkcustomLZ1 setMarkerText (format ["LZ %1", [_vehicle] call TREND_fnc_getTransportName]);
...
if (!_airEscort) then {
	...
	_AvoidZonePos = TREND_ObjectivePossitions select 0;
		...
		while {!_bEndSteps && _iSaftyCount > 0} do {
			...
			if (_stepDistToAO < 1000) then {
				if (_isHiddenObj) then {
					...
					_bEndSteps = true;
					_divertDirectionA = ([_DirAtoB,80] call TREND_fnc_AddToDirection);
					_newPosA = _AvoidZonePos getPos [2000,_divertDirectionA];
					_divertDirectionB = ([_DirAtoB,-80] call TREND_fnc_AddToDirection);
...
if (!([_vehicle] call TREND_fnc_helicopterIsFlying)) then {
	_locationText = [position _vehicle,true] call TREND_fnc_getLocationName;
	_text = format [localize "STR_TRGM2_transport_fnflyToLz_ClearTakeoff", [_vehicle] call TREND_fnc_getTransportName,_locationText];
	[_text] call TREND_fnc_commsHQ;
};
...
if (!([_vehicle] call TREND_fnc_helicopterIsFlying)) then {
	...
	[_vehicle,localize "STR_TRGM2_transport_fnflyToLz_OffWeGo"] call TREND_fnc_commsPilotToVehicle;
} else {
	[_vehicle,localize "STR_TRGM2_transport_fnflyToLz_Diverting"]call TREND_fnc_commsPilotToVehicle;
};
...
if (!_isPickup) then {
	[_vehicle, localize "STR_TRGM2_transport_fnflyToLz_ReachLZ_Out"] call TREND_fnc_commsPilotToVehicle;
}
else {
	[driver _vehicle,localize "STR_TRGM2_transport_fnflyToLz_ReachLZ_In"] call TREND_fnc_commsSide;
};
...
if (!_isPickup) then {
	...
	if (!(_thisMission call TREND_fnc_checkMissionIdActive)) then {
		...
		[_vehicle, localize "STR_TRGM2_transport_fnflyToLz_WelcomeAboard"] call TREND_fnc_commsPilotToVehicle;
	};
};

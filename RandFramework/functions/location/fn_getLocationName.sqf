...
if (_position distance2D _locationPosition > _distanceConsideredInArea) then {
	...
	_text = format [localize "STR_TRGM2_civillians_fngetLocationName_DirOfName",[_relDir,true] call TRGM_fnc_directionToText,_locationName];
} else {
	_format =  ["%1",localize "STR_TRGM2_civillians_fngetLocationName_At"] select _useAtLocation;

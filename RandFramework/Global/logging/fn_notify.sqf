...
if (_durationOrCondition isEqualType {}) then {
	[_content, 86400, 10, _notificationIndex, _durationOrCondition] spawn TRGM_GUI_fnc_handleNotification;
} else {
	[_content, _durationOrCondition, 10, _notificationIndex] spawn TRGM_GUI_fnc_handleNotification;
};

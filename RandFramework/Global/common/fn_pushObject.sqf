...
if (count crew _Object > 0) then {
    [(localize "STR_TRGM2_PushObject_PushEmpty")] call TRGM_GLOBAL_fnc_notify;
}
else {
    if (_caller isEqualTo player) then {
        ...
        _actionReleaseObject = player addAction [format [localize "STR_TRGM2_openDialogRequests_VehicleRelease", getText (configFile >> "CfgVehicles" >> (typeOf _Object)>> "displayName")], {
            ...
            [_target, [format [localize "STR_TRGM2_UnloadDingy_push", getText (configFile >> "CfgVehicles" >> (typeOf _target) >> "displayName")],{_this spawn TRGM_GLOBAL_fnc_pushObject;}, [], -99, false, false, "", "_this isEqualTo player && count crew _target isEqualTo 0"]] remoteExec ["addAction", 0];
        }, [_Object], 5, true, true];

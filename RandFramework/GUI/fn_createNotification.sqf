...
if (isNull (uiNamespace getVariable ["TRGM_VAR_notifications_disp", displayNull])) then { call TRGM_GUI_fnc_initNotifications; };
...
if !(isNull _ctrlGroup) then {
    ...
    [_ctrlGroup controlsGroupCtrl 1000, _duration, _condition] spawn {
        ...
        [ctrlParentControlsGroup _textControl] call TRGM_GUI_fnc_deleteNotification;
    };
};
